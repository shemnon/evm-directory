...
def gen_precompiles(chains):
    ...
    L = ["# Precompiles across chains", "",
         ...
         "The mainnet base range `0x01`–`0x11` plus P256VERIFY at `0x0100` is shown for "
         "every chain, verified against its pinned clone (`=` identical, `➖` absent, "
         "`⚠️` divergent). Every other row lists only the chains that diverge there; a "
         "blank cell in those rows means the chain has no entry at that address.",
         "", header(chains, slugs)]
...
def gen_txtypes(chains):
    ...
    L = ["# Transaction types across chains", "",
         ...
         "RLP list prefixes.", "",
         "The envelope `0x00`–`0x04` (Legacy, 2930, 1559, 4844, 7702) is shown for every "
         "chain, from an `envelope` each row declares (`=` accepted in mainnet shape, "
         "`➖` not accepted). Rows above `0x04` list only the chains that define a byte "
         "there.", "", header(chains, slugs)]
    for a in sorted(tab, key=sortkey):
        row = [cell(tab[a].get(s), s, org.get((a, s))) for s in slugs]
        nm = entry_label(tab[a].values())
        L.append(f"| `{a}` {nm} | " + " | ".join(row) + " |")
