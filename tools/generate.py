...
def header(chains, slugs):
    h = "| Address | " + " | ".join(name(chains[s]).replace("Avalanche ", "Avax ") for s in slugs) + " |"
    return h + "\n|" + "---|" * (len(slugs) + 1)
...
def legend():
    return ("\nLegend: ➕ added · ➖ removed/absent · ⚠️ modified (same address, different "
            "semantics) · = inherited · ◌ pending · ◐ opt-in · † inherited from a stack ancestor\n")
...
def gen_matrix(chains):
    ...
    L = ["# EVM difference matrix", "",
         "Generated by `tools/generate.py` — do not edit by hand.", "",
         "| | " + " | ".join(name(chains[s]) for s in slugs) + " |",
         "|---|" + "---|" * len(slugs)]
    def row(label, fn):
    ...
    row("EIP-7702", lambda c, s: {"inherited": "yes"}.get(
        (c.get("eips") or {}).get(7702, {}).get("status", "inherited"), "**no**"))
    row("Blobs (4844)", lambda c, s: {"inherited": "yes"}.get(
        (c.get("eips") or {}).get(4844, {}).get("status", "inherited"), "no"))
